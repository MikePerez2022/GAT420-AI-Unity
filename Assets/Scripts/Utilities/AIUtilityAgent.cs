...
public class AIUtilityAgent : Agent
{
	...
	public float happiness
	{
		get
		{
			...
			var happinessLvl = total / needs.Length;
			...
			return 1 - happinessLvl; // 1 - (divide total motives by number of needs to get average)
		}
	}
    ...
    void Update()
    {
		if (!Sad && happiness < 0.1 && Time.time >= timer)
		{
			timer += Time.time + 12;
			Sad = true;
			Movement.Stop();
			StartCoroutine(CheckLowHappiness());
		}
		else if (!Sad && Time.time >= timer)
		{
			timer = 0;
		}
	...
	IEnumerator CheckLowHappiness()
	{

		if (Sad) animator.SetBool("Sad", true);
